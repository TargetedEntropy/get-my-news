...
class ScraperStats:
    ...
    def get_summary_stats(self, days: int = 30) -> Dict[str, Any]:
        ...
        successful_runs = sum(
            1 for stats in recent_stats if stats.get("success", False)
        )
        total_articles = sum(
            stats.get("articles_processed", 0) for stats in recent_stats
        )
        ...
        return {
            ...
            "average_articles_per_run": total_articles / len(recent_stats)
            if recent_stats
            else 0,

...
class ProcessLock:
    ...
    def __init__(
        self,
        lock_file: str = "/tmp/newsfilter_scraper.lock",
        timeout: int = 300,
            ...
            try:
                ...
                if self.lock_file.exists():
                    if not self._is_process_running():
                        ...
                        if not wait:
                            self.logger.info(
                                "Another scraper instance is already running"
                            )
                            return False
                        ...
                        if time.time() - start_time >= self.timeout:
                            self.logger.error(
                                f"Timeout waiting for lock after {self.timeout} seconds"
                            )
                            return False

                        self.logger.info(
                            f"Waiting for lock... (PID: {self._get_lock_pid()})"
                        )
    ...
    def force_release(self):
        ...
        try:
            if self.lock_file.exists():
                ...
                self.lock_file.unlink()
                self.logger.warning(
                    f"Force released lock file (was PID: {lock_info.get('pid') if lock_info else 'unknown'})"
                )
        except OSError as e:

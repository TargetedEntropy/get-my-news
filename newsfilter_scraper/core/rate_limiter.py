...
class RateLimiter:
    ...
    def __init__(self, max_requests: int = 100, tracking_file: str = "data/rate_limit.json", 
                 reset_hour: int = 0):
        """
            ...
            if self.tracking_file.exists():
                ...
                if not isinstance(self.data, dict):
                    ...
                    if self.data['last_reset']:
                        self.data['last_reset'] = datetime.fromisoformat(self.data['last_reset'])
            else:
                self._initialize_tracking_data()
                
        except (json.JSONDecodeError, ValueError, OSError) as e:
            self.logger.warning(f"Could not load rate limit data: {e}. Initializing fresh data.")
            self._initialize_tracking_data()
    ...
    def _check_reset_needed(self):
        ...
        if not self.data['last_reset'] or current_reset_time > self.data['last_reset']:
            ...
            self.logger.info(f"Rate limit counter reset. Previous usage: {old_usage}/{self.max_requests}")
    ...
    def record_request(self):
        ...
        self.logger.debug(f"API request recorded. Usage: {self.data['daily_usage']}/{self.max_requests}")
    ...
    def force_reset(self):
        ...
        self.logger.warning(f"Rate limit counter force reset. Previous usage: {old_usage}/{self.max_requests}")

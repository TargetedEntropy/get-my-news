...
class NewsfilterScraper:
    ...
    def _check_rate_limits(self) -> bool:
        ...
        try:
            if self.rate_limiter.can_make_request():
                ...
                self.logger.info(f"Rate limit check passed: {current_usage['daily_usage']}/{current_usage['max_requests']} calls used today")
                return True
            else:
                current_usage = self.rate_limiter.get_current_usage()
                self.logger.warning(f"Rate limit exceeded: {current_usage['daily_usage']}/{current_usage['max_requests']} calls used today")
                return False
        except Exception as e:
    ...
    def _process_articles(self, session, articles_data: List[Dict[str, Any]]) -> bool:
        ...
        try:
            for article_data in articles_data:
                try:
                    ...
                    if existing_article:
                        ...
                        self.logger.debug(f"Article {article_data['id']} already exists, skipping")
                        continue
                    ...
                    self.logger.error(f"Failed to process article {article_data.get('id', 'unknown')}: {str(e)}")
            ...
            # Commit all changes
            session.commit()
            self.logger.info(f"Successfully committed {self.stats.articles_processed} new articles to database")
    ...
    def _create_article_from_data(self, session, article_data: Dict[str, Any]) -> Article | None:
    ...
    def _log_final_stats(self, start_time: datetime, success: bool):
        ...
        self.logger.info(f"Articles skipped (duplicates): {self.stats.articles_duplicate}")
        ...
        current_usage = self.rate_limiter.get_current_usage()
        self.logger.info(f"API usage today: {current_usage['daily_usage']}/{current_usage['max_requests']}")
        ...
        print(f"API calls used: {current_usage['daily_usage']}/{current_usage['max_requests']}")
